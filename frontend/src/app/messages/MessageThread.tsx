...
const MessageThread: React.FC<MessageThreadProps> = ({
  ...
  const fetchMessages = async (newPage: number) => {
    if (!isAuthenticated) {
      setErrorMessage(
        "You must be logged in to fetch messages. Redirecting to login.",
      );
      setModalOpen(true);
      setTimeout(() => {
        router.push("/login");
  ...
  const sendMessage = async () => {
    if (!isAuthenticated) {
      setErrorMessage(
        "You must be logged in to send messages. Redirecting to login.",
      );
      setModalOpen(true);
      setTimeout(() => {
        router.push("/login");
                        ...
                        {!isCurrentUser && (
                          <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8 border border-orange-300 mr-2">
                            <img
                              ...
                              alt="Recipient"
                              src={getFullImageUrl(
                                receiverInfo?.profileImageUrl,
                              )}
                            />
                          </span>
                        )}
                        <div
                          className={`max-w-[70%] rounded-lg p-3 ${isCurrentUser ? "bg-orange-500 text-white" : "bg-orange-100 text-orange-700"}`}
                        >
                          <p>{message.content}</p>
                          <p
                            className={`text-xs mt-1 ${isCurrentUser ? "text-orange-100" : "text-orange-500"}`}
                          >
