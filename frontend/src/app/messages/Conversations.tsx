...
const Conversations: React.FC<UpdatedConversationsProps> = ({
  userId,
  conversations,
  setConversations,
  ...
  const fetchConversationsAndUnreadMessages = async () => {
    ...
    try {
      ...
      const unreadMessagesByConversation = unreadMessagesResponse.data.reduce(
        (acc, message) => {
          acc[message.conversation.id] =
            (acc[message.conversation.id] || 0) + 1;
          return acc;
        },
        {} as Record<number, number>,
      );

      const conversationsWithUnread = conversationsResponse.data.map(
        (conversation) => ({
          ...conversation,
              ...
              conversations.map((conversation: ConversationWithUnread) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      handleConversationClick(conversation.id, otherUser.id)
                    }
                  >
                    <span className="relative flex shrink-0 overflow-hidden rounded-full w-10 h-10 border">
                      <img
                        ...
                        src={
                          getFullImageUrl(otherUser.profileImageUrl) ||
