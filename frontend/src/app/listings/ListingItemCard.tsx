...
const ListingItemCard: React.FC<ListingItemCardProps> = ({
  item,
  getFullImageUrl,
  isSelected,
  onToggleSelect,
  ...
  >(item.imageUrls.map((url) => ({ url, isNew: false })));
  ...
  const fetchBuyers = async () => {
    try {
      ...
      const filteredBuyers = response.data.filter(
        (user: any) => user.id !== item.seller.id,
      );
  ...
  const handleAddTag = () => {
    if (
      newTag &&
      !editedItem.tags.includes(newTag) &&
      editedItem.tags.length < 5
    ) {
      setEditedItem((prev) => ({
        ...prev,
  ...
  const handleDoubleClick = () => {
    if (!isSold) {
      ...
      toast({
        ...
        description:
          "You cannot edit sold items. Only active items on the marketplace can be edited.",
        variant: "default",
        duration: 5000,
      });
    }
  ...
  const nextImage = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex + 1) % item.imageUrls.length,
    );
  };
  ...
  const prevImage = () => {
    setCurrentImageIndex(
      (prevIndex) =>
        (prevIndex - 1 + item.imageUrls.length) % item.imageUrls.length,
    );
  };
    ...
    <div>
        ...
        <div className="relative h-48 flex-shrink-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="absolute top-1 right-1 p-1 rounded-full bg-white/70"
              >
        ...
        <div className="p-3 bg-gradient-to-b from-gray-50 to-gray-100 flex-grow flex flex-col justify-between">
          <div>
            ...
            <div className="flex flex-wrap gap-1 mb-1">
              {item.tags.slice(0, 2).map((tag, index) => (
                <span
                  key={index}
                  className="bg-orange-100 text-orange-800 text-xs px-1 py-0.5 rounded-full"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="mt-2 flex items-center justify-between text-xs">
            ...
            {isSold && item.purchaseDate && (
              <p className="text-gray-500">
                Sold: {new Date(item.purchaseDate).toLocaleDateString()}
              </p>
            )}
            {!isSold && (
              <Button
      ...
      <Dialog open={isEditModalOpen} onOpenChange={handleCloseEditModal}>
        <DialogContent className="sm:max-w-[625px] bg-orange-50 border-2 border-orange-300">
          <DialogHeader>
            <DialogTitle className="text-center text-2xl font-bold text-orange-700">
              Edit Listing
            </DialogTitle>
          </DialogHeader>
      ...
      >
        <AlertDialogContent className="bg-orange-50 border-2 border-orange-300">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-gray-200 text-orange-700 hover:bg-gray-300">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <Dialog
        ...
        onOpenChange={setIsMarkAsSoldModalOpen}
      >
                    ...
                    >
                      <div className="flex flex-row items-center space-x-2 w-full">
                        <Avatar className="h-8 w-8 flex-shrink-0">
                          <AvatarImage
                            src={getFullProfileImageUrl(buyer.profileImageUrl)}
                        ...
                        <div className="flex flex-row items-center space-x-2 flex-grow min-w-0">
                          <p className="text-sm text-orange-700 whitespace-nowrap">
