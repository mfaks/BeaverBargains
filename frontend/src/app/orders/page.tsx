...
export default function Orders() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await axios.get<OrderItem[]>(
        "https://beaverbargains.onrender.com/api/items/user/purchased",
        {
          headers: {
      ...
      const initialIndices = response.data.reduce(
        (acc, item) => {
          acc[item.id] = 0;
          return acc;
        },
  ...
  const nextImage = (orderId: number) => {
    setCurrentImageIndices((prevIndices) => ({
      ...prevIndices,
      [orderId]:
        (prevIndices[orderId] + 1) %
        orders.find((order) => order.id === orderId)!.imageUrls.length,
    }));
  };
              ...
              <main className="container mx-auto px-4 md:px-6 py-12">
                ...
                {loading ? (
                  ...
                  <EmptyStateCard
                    ...
                    actionText={
                      orders.length > 0 ? "Clear Filters" : "Go to Marketplace"
                    }
                    onAction={
                      orders.length > 0
                        ? handleClearFilters
